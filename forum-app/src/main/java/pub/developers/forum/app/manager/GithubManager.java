...
public class GithubManager extends AbstractLoginManager {
    ...
    public String oauthLogin(GithubOauthLoginRequest request) {
        ...
        JsonNode githubUser = githubService.getUserInfo(request.getCode());

        String loginName = githubUser.get("login").asText();
        String email = ObjectUtils.isEmpty(githubUser.get("email")) ? loginName + "@github.com" : githubUser.get("email").asText();
        String nickname = ObjectUtils.isEmpty(githubUser.get("name")) ? loginName : githubUser.get("name").asText();
        String signature = ObjectUtils.isEmpty(githubUser.get("bio")) ? "" : githubUser.get("bio").asText();
        String avatar = ObjectUtils.isEmpty(githubUser.get("avatar_url")) ? "" : githubUser.get("avatar_url").asText();
